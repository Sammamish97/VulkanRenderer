...
void VkApp::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout,
...
void VkApp::createDepthResources(VkImage depthImage, VkDeviceMemory memory)
{
	//TODO: Temporary code.
...
void VkApp::mouseCallback(GLFWwindow* window, double xposIn, double yposIn)
{
	auto app = reinterpret_cast<VkApp*>(glfwGetWindowUserPointer(window));
...
void VkApp::createGraphicsPipelines()
{
	...
	shaderStages[0] = mGVertShader->GetShaderStageCreateInfo(vulkanDevice->logicalDevice);
	shaderStages[1] = mGFragShader->GetShaderStageCreateInfo(vulkanDevice->logicalDevice);
...
VkImageView VkApp::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
{
...
void VkApp::createAttachment(VkFormat format, VkImageUsageFlagBits usage,
